...
try:
    ...
    print(
        "If the .venv doesn't exist, run: .\\setup-venv.ps1 (Windows) or python setup-venv.ps1 (Linux/Mac)",
        file=sys.stderr,
    )
    sys.exit(1)
...
def verify_deployment(handler, template_name, verbose=False, max_wait_time=600, check_interval=5):
    ...
    try:
        ws_resp = requests.get(
            f"{api_url}/api/v2/workspaces", headers=headers, verify=False, timeout=30
        )
        ...
        if isinstance(ws_data, list):
            ...
            workspaces = [
                w for w in ws_data.get("workspaces", []) if w.get("template_name") == template_name
            ]
        else:
            workspaces = []
    ...
    for ws in workspaces:
        ...
        try:
            ws_detail_resp = requests.get(
                f"{api_url}/api/v2/workspaces/{ws_id}", headers=headers, verify=False, timeout=10
            )
        ...
        if ws_status not in ["running", "stopped"]:
            print(
                f"    [INFO] Workspace build status is '{ws_status}' - waiting for it to complete..."
            )
            # Don't mark as unhealthy yet if build is still in progress
            if ws_status in ["pending", "starting", "stopping"]:
                print("    [INFO] Build is in progress, will wait for completion")
            else:
                print(f"    [WARN] Unexpected workspace status: '{ws_status}'")
                all_healthy = False

        if not ws_health and ws_status == "running":
            print("    [WARN] Workspace health check failed (but status is running)")
            ...
            try:
                ...
                if agents_resp.status_code == 404:
                    if elapsed - last_status_print >= 15:  # Print every 15 seconds
                        print(
                            f"    Waiting for agents to appear (elapsed: {elapsed}s/{max_wait_time}s)..."
                        )
                ...
                if agents:
                    ...
                    if elapsed - last_status_print >= 15:
                        print(
                            f"    Agent status: {agent_status} (waiting for 'connected' or 'ready'...)"
                        )
                        last_status_print = elapsed
                ...
                if e.response.status_code == 404:
                    ...
                    if elapsed - last_status_print >= 15:
                        print(
                            f"    Waiting for agents endpoint (404, elapsed: {elapsed}s/{max_wait_time}s)..."
                        )
                        last_status_print = elapsed
                elif verbose:
        ...
        if not agent_connected:
            print(
                f"    [WARN] Agent did not connect within {max_wait_time}s ({max_wait_time // 60}min)"
            )
            print(
                "    [INFO] This may be normal if the workspace was just restarted - agents can take several minutes"
            )
            print("    [INFO] The workspace status shows as 'running' - agent may connect later")
            print("    [INFO] Check workspace status manually in the Coder UI")
        ...
        try:
            ...
            if agents_resp.status_code == 404:
                print(
                    "    [INFO] Agents endpoint returned 404 - agents not created yet (normal after restart)"
                )
                print(
                    "    [INFO] Workspace may need more time - agents typically appear within 1-2 minutes after restart"
                )
                all_healthy = False
                continue
                ...
                for app in apps:
                    ...
                    if app_slug == "app" and app_url:
                        try:
                            ...
                            print("        [WARN] App request timed out")
                            all_healthy = False
                        except Exception as e:
                    ...
                    if app_health not in ["ok", "healthy"]:
                        print(
                            f"        [WARN] App health is '{app_health}', expected 'ok' or 'healthy'"
                        )
                        all_healthy = False
            else:
                print("    [WARN] No agents found (should have been connected)")
                all_healthy = False
        except Exception as e:
...
def parse_args():
    ...
    # Required arguments
    parser.add_argument("--remote-user", type=str, required=True, help="Remote SSH user (required)")
    parser.add_argument("--remote-host", type=str, required=True, help="Remote SSH host (required)")
    parser.add_argument(
        "--remote-base-dir",
        type=str,
        required=False,
    ...
    parser.add_argument("--template-name", type=str, required=True, help="Template name (required)")
    parser.add_argument(
        "--namespace",
        type=str,
        default="coder",
    ...
    parser.add_argument(
        "--remote-scripts-dir", type=str, default=None, help="Remote scripts directory (optional)"
    )
    parser.add_argument(
        "--remote-template-dir", type=str, default=None, help="Remote template directory (optional)"
    )
...
def main():
    ...
    if not coder_api_token and not args.no_auto_token:
        ...
        try:
            ...
            if auto_token:
            ...
            elif coder_email and coder_password:
                print(
                    "[WARN] Could not automatically get token, will use email/password authentication"
                )
            else:
                ...
                print(
                    "    - OR ensure you're logged in on the remote server: ssh user@server 'coder login'"
                )
        except Exception as e:
    ...
    try:
        ...
        if args.remote_base_dir:
            ...
            print("  Remote Base Dir: (will use temporary directory)")
        ...
        if not args.skip_connectivity_checks:
            ...
            if not handler.check_internet_access():
                print(
                    "  [WARN] Internet access check failed (may be expected in air-gapped environments)"
                )
            else:
            ...
            if not checks_passed:
                if getattr(args, "bypass_interaction", False):
                    ...
                    print(
                        "  [WARN] Some connectivity checks failed, but continuing with deployment..."
                    )
        ...
        try:
            ...
            if result == 0:
                ...
                if verification_result:
                    print()
                    print(
                        "[OK] Deployment verification passed - all workspaces, agents, and apps are healthy!"
                    )
                else:
                    print()
            ...
            if "NoneType" in error_msg and "not a container" in error_msg:
                print(
                    "[WARN] Output processing error (cosmetic):",
                    error_msg.encode("ascii", "replace").decode("ascii"),
                )
