...
from _utils.utils import git, teams, misc, sql
...
class GlueHandler:
    ...
    def get_current_glue_job_metadata(self, job_name:str):
        ...
        if not job_name:
            raise ValueError("JOB_NAME environment variable not set. Ensure this script runs inside a Glue job.")
    ...
    def create_glue_job(
        ...
        if source_control_details:
            if not github_token_secret_name:
                raise ValueError("github_token_secret_name parameter is required when source_control_details is provided")
            ...
            s3_path = f"{source_control_details.get('Repository')}/{source_control_details.get('Folder')}"
        ...
        try:
            ...
            logger.error(f"Failed to create Glue job {job_name}: {e}")
            raise

    def trigger_glue_job(self, job_name: str, job_args: Optional[Union[str, Dict[str, Any]]] = None) -> str:
        """
        ...
        try:
            if isinstance(job_args, str):
                try:
                    ...
                    logger.error(f"Invalid JSON string for job_args: {e}")
                    raise ValueError("job_args must be a valid JSON string or a dictionary.")

            # Ensure all job arguments are strings
            if job_args:
                job_args = {k: json.dumps(v) if not isinstance(v, str) else v for k, v in job_args.items()}
            ...
            logger.error(f"Failed to trigger Glue job {job_name}: {e}")
            raise

    async def monitor_glue_job(self, job_name: str, job_run_id: str, interval: int = 30) -> Dict[str, Any]:
        import asyncio
        """
        ...
        while True:
            try:
                ...
                logger.error(f"Error checking Glue job status: {e}")
                raise
    ...
    def generate_logstream_urls(self, job_name, job_run_id) -> dict:
        ...
        try:
            ...
            output_url = (
                f"{base_url}/$252Faws-glue$252Fjobs$252Foutput"
                f"/log-events/{log_stream_name}"
            )

            error_url = (
                f"{base_url}/$252Faws-glue$252Fjobs$252Ferror"
                f"/log-events/{log_stream_name}"
            )
            ...
            logger.error(f"Error generating log stream URLs: {e}")
            return {'output': None, 'error': None}
    ...
    def log_job(self,job_meta_data, error = ''):
        ...
        if sql_response.empty:
            if error: job_meta_data['job_error'] = json.dumps(error)
            log_insert_stmt = sql.dict_to_insert_stmt(data  =job_meta_data, schema = 'glue', table = 'logs')
            ...
            if error: error = json.dumps(error).replace("'", "''") if error else ''
    ...
    def process_input(self, argv: dict):
        ...
        for key,value in argv.items():
            ...
            if key.startswith('--'):
                ...
                if key.lower() in ["--log_id","--job_id","--job_run_id","--job_name","--tempdir"]:
        ...
        print('UPDATES')
        meta_dict = misc.flatten_dict(self.get_current_glue_job_metadata(job_name=job_meta_data.get("job_name")))
        print('META DICT',meta_dict)
        job_meta_data.update({k.lower():v for k,v in misc.flatten_dict(self.get_current_glue_job_metadata(job_name=job_meta_data.get("job_name"))).items()})
        print('here?')
        job_meta_data.update({k+'_url':v for k, v in self.generate_logstream_urls(job_name= job_meta_data.get("job_name"), job_run_id= job_meta_data.get("job_run_id")).items()})
