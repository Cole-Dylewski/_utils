...
class CognitoHandler:
    ...
    def __init__(
        ...
        if not cognito_app_client_id: 
            if not cognito_creds_secret_name:
                raise ValueError("cognito_creds_secret_name parameter is required when cognito_app_client_id is not provided")
    ...
    def authenticate_user(self, username: str, password: str) -> dict:
        ...
        try:
            ...
            if auth_result:
                ...
                now = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3] + " " + datetime.now().astimezone().strftime("%z")
    ...
    def signout_user(self, refresh_token: str):
        ...
        try:
            ...
            response = self.cognito_client.revoke_token(
                Token=refresh_token,
                ClientId=self.client_id
            )
    ...
    def global_signout_user(self, access_token: str):
        ...
        try:
            ...
            logger.error(f"Error during global signout: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Global signout failed: {str(e)}")
    ...
    def force_password(self, username: str, new_password: str, session: str) -> dict:
        ...
        try:
            ...
            logger.error(f"Error during password reset challenge: {str(e)}")
    ...
    def confirm_forgot_password(self, username: str, confirmation_code: str, new_password: str) -> dict:
        ...
        try:
            ...
            logger.error(f"The confirmation code has expired: {str(e)}")
    ...
    def manage_user(self, username: str, email: str, first_name: str, last_name: str, password: str, 
                    role: Optional[str] = None, license_level: Optional[str] = None, action: str = "CREATE") -> dict:
        """
        ...
        try:
            if action == "CREATE":
                ...
                print(f"Password reset successfully for user {username}. User must change password at next login.")

            else:
                raise HTTPException(status_code=400, detail=f"Invalid action '{action}'. Use 'CREATE' or 'RESET'.")
            ...
            user_info = self.cognito_client.admin_get_user(UserPoolId=self.user_pool_id, Username=username)
            
            # Extract the Cognito user ID (sub attribute)
            cognito_user_id = next(
                (attr['Value'] for attr in user_info['UserAttributes'] if attr['Name'] == 'sub'), None
            )
            logger.info(f"Cognito User ID: {cognito_user_id}, Password Expiry: {password_expiry_time}")
            ...
            return {
                ...
                "created_at": user_creation_time.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3] + " -0000",
                "password_expiry_time": password_expiry_time.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M:%S.%f")[:-3] + " -0000"
            }

        except self.cognito_client.exceptions.UsernameExistsException as e:
            logger.error(f"User creation failed - Username already exists: {username}")
            raise HTTPException(status_code=409, detail="Username already exists")
        except self.cognito_client.exceptions.InvalidPasswordException as e:
            logger.error(f"User creation failed - Invalid password: {username}, {str(e)}")
            raise HTTPException(status_code=400, detail="Invalid password format")
        except Exception as e:
            logger.error(f"User creation failed for username: {username}, Error: {str(e)}")
    ...
    def get_user_by_email(self, email: str) -> dict:
        ...
        try:
            ...
            logger.error(f"Too many attempts. Please wait before making another request.: {str(e)}")
    ...
    def get_user_info(self, cognito_user_id: str) -> dict:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve information for Cognito user {cognito_user_id}: {str(e)}")
            raise HTTPException(status_code=500, detail="Failed to retrieve user information from Cognito")
    ...
    def update_user(self, username: str, updates: dict) -> dict:
        ...
        try:
            ...
            logger.info(f"Updating cognito User ID: {str(username)}")
            attribute_mapping = {                
            ...
            logger.error(f"User update failed - Username doesn't exists: {username}")
            raise HTTPException(status_code=404, detail="User not found")        
        except Exception as e:
    ...
    def enable_user(self, cognito_user_id: str) -> None:
        ...
        try:
            ...
            if user.get('Enabled', False):  # 'Enabled' is True
                ...
                return {"status": "already_enabled", "message": "User is already enabled"} # Exit the function, as no further action is needed
            ...
            logger.error(f"Failed to enable Cognito user {cognito_user_id}: {str(e)}")
    ...
    def disable_user(self, cognito_user_id: str) -> None:
        ...
        try:
            ...
            if not user.get('Enabled', True):  # If 'Enabled' is False
                ...
                return {"status": "already_disabled", "message": "User is already disabled"} # Exit the function, as no further action is needed
            ...
            logger.error(f"Failed to disable Cognito user {cognito_user_id}: {str(e)}")
    ...
    def delete_user(self, cognito_user_id: str) -> None:
        ...
        try:
            ...
            logger.error(f"Too many attepts. Please wait for sometime before making request.: {str(e)}")
            raise HTTPException(status_code=429, detail="Too many requests")
        except Exception as e:
    ...
    def admin_global_signout_user(self, access_token: str):        
        ...
        try:
            ...
            logger.error(f"Error during global signout: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Global signout failed by Admin: {str(e)}")  
    ...
    def get_user_auth_events_detailed(self, username: str, max_events: int = 10) -> dict:
        ...
        try:
            ...
            for event in auth_events_response.get("AuthEvents", []):
                ...
                event_risk = event.get("EventRisk", {})
    ...
    def get_all_user_auth_events_detailed(self, max_users: int = 100, max_events: int = 5) -> list:
        ...
        for page in paginator.paginate(UserPoolId=self.user_pool_id):
            for user in page.get("Users", []):
                username = user.get("Username")
                try:
                    user_summary = self.get_user_auth_events_detailed(username=username, max_events=max_events)
    ...
    def get_all_users_last_activity(self, max_users: int = 100) -> list:
        ...
        try:
            for page in paginator.paginate(UserPoolId=self.user_pool_id):
                for user in page.get("Users", []):
                    ...
                    try:
                        ...
                        last_event = auth_events.get("AuthEvents", [])[0]["CreationDate"] if auth_events.get("AuthEvents") else last_modified_time
                        last_activity = last_event.strftime("%Y-%m-%d %H:%M:%S.%f") if last_event else None

                    except self.cognito_client.exceptions.UserNotFoundException:
                        last_activity = None
                    except Exception as e:
                        logger.warning(f"Could not fetch activity for {username}: {str(e)}")
                        last_activity = None
            ...
            logger.error(f"Failed to fetch users' last activity: {str(e)}")
            raise HTTPException(status_code=500, detail="Failed to retrieve users' activity timestamps.")
