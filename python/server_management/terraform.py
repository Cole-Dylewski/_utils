...
class TerraformHandler:
    ...
    def __init__(
        ...
        logger.info(f"TerraformHandler initialized: project_dir={self.project_dir}, remote={self.is_remote}")
    ...
    def _run_remote_command(
        ...
        try:
            ...
            logger.error(f"Remote command timed out: {' '.join(cmd)}")
            raise
        except subprocess.CalledProcessError as e:
    ...
    def plan(
        ...
        if check_crd_errors and result.returncode != 0:
            ...
            is_crd_error = (
                "CRD may not be installed" in output or
                "no matches for kind" in output.lower() or
                "GroupVersionKind" in output or
                ("external-secrets.io" in output.lower() and "SecretStore" in output)
            )
        
        if result.returncode == 0:
            logger.info("Terraform plan completed successfully")
        elif is_crd_error:
            logger.warning("Terraform plan detected CRD-related errors (expected in some environments)")
        else:
    ...
    def state_list(self, addresses: Optional[List[str]] = None) -> List[str]:
        """
