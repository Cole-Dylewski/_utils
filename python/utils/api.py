...
async def send_async_request(
    ...
    **kwargs,
    ...
    request_params = _prepare_request_params(
        method, url, params, data, json, headers, timeout, **kwargs
    )
...
def send_sync_request(
    ...
    **kwargs,
    ...
    with httpx.Client() as client:
        return client.request(**request_params)


async def run_async_requests_in_parallel(requests: list[dict[str, Any]]) -> list[httpx.Response]:
    tasks = [send_async_request(**request_args) for request_args in requests]
...
def aws_api_request(
    ...
    if (not gatewayID) and (not apiKey):
        if not api_creds_secret_name:
            raise ValueError(
                "api_creds_secret_name parameter is required when gatewayID and apiKey are not provided"
            )
...
def multipart_upload_to_s3(file_path, bucket_name, folder_path, client):
    upload_id = initiate_multipart_upload(
        bucket_name, folder_path, os.path.basename(file_path), client
    )
    ...
    try:
        ...
        if response["ResponseMetadata"]["HTTPStatusCode"] == 200:
            ...
            return True
        print(
            f"Failed to complete multipart upload. Status code: {response['ResponseMetadata']['HTTPStatusCode']}"
        )
        return False
