...
class RedisHandler:
    def __init__(
        ...
        ssl: Optional[str] = os.getenv('REDIS_SSL')
    ):
        try:
    ...
    async def get_all_keys(self) -> List[str]:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error retrieving keys from Redis: {str(e)}")
    ...
    async def flush(self) -> str:
        ...
        try:
            ...
            logger.error(f"Error clearing Redis memory: {e}")
            raise HTTPException(status_code=500, detail=f"Error clearing Redis memory: {str(e)}")
    ...
    async def get_keys_without_ttl(self) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error retrieving keys without TTL: {e}")
            raise HTTPException(status_code=500, detail=f"Error retrieving keys without TTL: {str(e)}")
    ...
    async def condemn_keys(self, ttl: int = DEFAULT_TTL) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error setting TTL for keys without TTL: {e}")
            raise HTTPException(status_code=500, detail=f"Error setting TTL for keys: {str(e)}")
    ...
    async def set_ttl_for_key(self, key: str, ttl: int) -> bool:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error setting TTL for key: {str(e)}")
               
    async def set_key(self, key: str, obj: Dict[str, Any], ttl: Optional[int] = DEFAULT_TTL) -> None:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error storing key: {str(e)}")
    ...
    async def get_key(self, key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error retrieving metadata for key '{key}': {e}")
            raise HTTPException(status_code=500, detail=f"Error retrieving metadata from Redis: {str(e)}")
    ...
    async def get_total_memory_usage(self) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                ...
                "Max Memory Configured": self._format_memory(max_memory) if max_memory > 0 else "Unlimited",
                "Memory Fragmentation Ratio": f"{memory_info.get('mem_fragmentation_ratio', 'N/A')}"
            }
        except Exception as e:
            logger.error(f"Error retrieving total memory usage from Redis: {e}")
            raise HTTPException(status_code=500, detail=f"Error retrieving memory usage: {str(e)}")
    ...
    async def publish_to_room(self, room_name: str, message: str) -> int:
        ...
        try:
            ...
            room_val['value']['messages'][dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")]=message
            ...
            logger.error(f"Failed to publish message to room '{room_name}': {e}")
    ...
    async def subscribe_to_room(self, room_name: str) -> PubSub:
        ...
        try:
            ...
            await self.publish_to_room(room_name, f"A user has joined the room")
            logger.info(f"Subscribed to room: {room_name}")
            return pubsub
        except Exception as e:
            logger.error(f"Failed to subscribe to room '{room_name}': {e}")
            raise HTTPException(status_code=500, detail=f"Subscribe failed: {str(e)}")
    ...
    async def unsubscribe_from_room(self, pubsub: PubSub, room_name: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to unsubscribe from room '{room_name}': {e}")
    ...
    async def listen_to_room(self, pubsub: PubSub, callback):
        ...
        try:
            ...
            logger.error(f"Error while listening to room: {e}")
            raise
