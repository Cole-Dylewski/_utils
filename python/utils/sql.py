...
from _utils.aws import  secrets, s3, aws_lambda
...
async def ping_multiple_databases(databases: dict) -> list:
    ...
    for rds,db in databases.items():
        try:
            ...
            logger.error(f"Database connection failed for {db.get('host', 'unknown')}: {e}")
            results.append({
                "database": db.get('host', 'unknown'),
                "status": "failed",
                "error": str(e)
            })
            ...
            if isinstance(query, list):
                ...
                for q in query:
                    if queryType.lower() == 'query':
                        ...
                        if result.startswith('UPDATE') or result.startswith('DELETE') or result.startswith('INSERT'):
        ...
        with PostgreAdapter.connect(**creds) as conn:
            with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                ...
                try:
                    ...
                    logger.error(f"Error executing SQL, rollback triggered: {query_error}")
                    raise
...
where table_id = '{table_id}';"""
    ...
    return table_meta['rds'], table_meta['table_catalog'], table_meta['table_schema'], table_meta['table_name']   
...
async def get_table_defs(rds:str, dbname:str,schema:str, table:str):
    ...
    response =  await run_sql_async(
        query=table_def_query,
        queryType='query',
        dbname=dbname,
        rds=rds
    )
...
def sanitize_value(value):
    ...
    if isinstance(value, str):
        return "'" + value.replace("'", "''") + "'"  # Manually construct the string to avoid f-string issues
    
    return str(value)


async def validate_data(data: Dict[str, Any], rds:str, dbname:str,schema:str, table:str, operation: str):
    """
    ...
    for column_name, value in data.items():
        ...
        if data_type in ['integer', 'bigint']:
            if not isinstance(value, int):
                errors.append(f"Column '{column_name}' expects an integer but got {type(value).__name__}.")
        elif data_type == 'character varying':
            if not isinstance(value, str):
                errors.append(f"Column '{column_name}' expects a string but got {type(value).__name__}.")
            elif max_length and len(value) > max_length:
                errors.append(f"Column '{column_name}' value length exceeds maximum of {int(max_length)} characters.")
        elif data_type == 'boolean':
            if not isinstance(value, bool):
                errors.append(f"Column '{column_name}' expects a boolean but got {type(value).__name__}.")
        elif data_type in ['numeric', 'decimal']:
            if not isinstance(value, (int, float)):
                errors.append(f"Column '{column_name}' expects a numeric type but got {type(value).__name__}.")
                ...
                if len(integer_part_value) > integer_part or len(decimal_part_value) > decimal_part:
                    errors.append(f"Column '{column_name}' value '{value}' exceeds numeric precision {numeric_precision} and scale {numeric_scale}.")

        # Check for NOT NULL constraints; ignore for updates if the value is not provided
        if value is None and not is_nullable:
            if operation.upper() == 'INSERT' or (operation.upper() == 'UPDATE' and column_name in data):
                errors.append(f"Column '{column_name}' cannot be NULL as it is defined as NOT NULL.")
...
def df_to_create_stmt(rds, df, schema, table, batch_bool=False):
    ...
    for col in df.columns:
        ...
        if dtype == 'object' and pd.notna(max_char):
            varchar_limit = next(filter(lambda x: x > max_char, varchar_lengths), varchar_lengths[-1])
...
def df_to_insert_stmt(df, table_name, nullify=[], parse_data_types=True, strip=False):
    """Generates a bulk INSERT SQL statement from a DataFrame."""
    df.replace(to_replace=nullify, value=np.nan, inplace=True)
    ...
    return insert_stmt
# %% COPY STMT
def create_s3_copy_stmt(bucket, key, rds, schema, table, aws_secret, column_map='', delimiter=',', region='us-east-1'):
...
def create_s3_upload_stmt(bucket, key, rds, schema, table, columnMap = '',delimiter = '', region = 'us-east-1', aws_redshift_access_key_secret_name = ''):
...
def export_qry_to_s3(rds, query, bucket, object_key, delimiter=',', region='us-east-1', return_s3_info=False):
...
def migrate_data(sourceRDS, sourceDbName, sourceQuery, targetRDS, targetDbName, targetSchema, targetTable, requestType, payload = {}, runType = 'sdk', qualifier = '', formatFile = False):
    if not qualifier:
        raise ValueError("qualifier parameter is required")
     
    #EXPORT THE DATA TO S3
    fileName = f"{sourceRDS}.{sourceDbName} to {targetRDS}.{targetDbName} {targetSchema}.{targetTable}.csv"
    
    #Extract the data to a CSV
    s3ExportQuery, bucket, objectKey = export_qry_to_s3(rds = sourceRDS, query = sourceQuery, fileName = fileName, returnS3Info = True)
...
def s3_to_rds(bucket, key, dbname, rds, schema ,table, delimiter = '', columnMap = '',lineterminator = '' ):
    secret = get_rds_secret(rds)
    uploadStmt = create_s3_copy_stmt(bucket, key, rds, schema, table, columnMap = columnMap, delimiter = delimiter)
