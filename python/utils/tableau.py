...
class tableau_client:
    def __init__(
        ...
        api_version=3.15,
        printVerbose=False,
        tableau_creds_secret_name="",
        tableau_creds_2025_secret_name="",
    ):
        ...
        if not username or not password or not server_url:
            if not tableau_creds_secret_name:
                raise ValueError(
                    "tableau_creds_secret_name parameter is required when username, password, or server_url are not provided"
                )
    ...
    def get_site(self, site=""):
        ...
        {k.lower(): v for k, v in json.loads(response.text).get("sites").items()}
        ...
        if site:
            ...
            return None
        return {
            siteInfo.get("name").lower(): siteInfo
            for siteInfo in json.loads(response.text).get("sites").get("site")
        }
    ...
    def get_meta_data(
            ...
            while not done:
                ...
                if response.status_code == 200:
                    ...
                    if response_json.get("pagination", False):
                        ...
                        page_number += 1
                        if total_returned >= int(
                            response_json.get("pagination").get("totalAvailable")
                        ):
                            done = True
                    else:
                        done = True
                    records += [
                        flatten_dict(record, key=subType)
                        for record in response_json.get(f"{subType}s").get(subType)
                    ]
                else:
        ...
        if workbook_id and "workbook" in subTypes:
            results["workbook"] = results["workbook"][
                results["workbook"]["workbook_id"] == workbook_id
            ]
        ...
        if datasource_id and "datasource" in subTypes:
            results["datasource"] = results["datasource"][
                results["datasource"]["datasource_id"] == datasource_id
            ]
        ...
        if merge:
            ...
            for i, key in enumerate(keys):
                if i == 0:
                    ...
                    results["output"] = results["output"].merge(
                        results[key], left_on=leftOn, right_on=rightOn, how="left"
                    )
    ...
    def download_view(
        self, outputType, view_id, subType="view", filter=False, pdfparameters=False, pivot=True
    ):
        import requests
        ...
        if filter:
            for k, v in filter.items():
                if v:
                    parameters.append(
                        f"vf_{parse.quote(str(k), safe='')}={parse.quote(str(v), safe='')}"
                    )
        ...
        if df:
            ...
            if pivot and "Measure Values" in df.columns.to_list():
                df["Measure Values"] = [
                    float(str(n).replace("$", "").replace("%", "").replace(",", ""))
                    for n in df["Measure Values"].to_list()
                ]
                cols = [
                    c for c in df.columns.values if c not in ("Measure Values", "Measure Names")
                ]
                df = pd.pivot(
                    df, values="Measure Values", columns="Measure Names", index=cols
                ).reset_index()

            return df
        return response

    def archive_reports(self, bucket, folder, files=None):
    ...
    def get_user(self):
        url = (
            f"{self.server_url}/api/{self.api_version}/sites/{self.credentials['site']['id']}/users"
        )
        response = requests.get(
            url,
            headers={
    ...
    def generate_report(
        ...
        view_ids = [
            {
                "view_id": view_id,
                "viewURL": self.get_meta_data(subType=["view"], output="json", view_id=view_id)[
                    0
                ].get("view_viewUrlName", ""),
            }
        ...
        if isinstance(filters, dict):
            ...
            response = self.download_view(
                outputType="data", view_id=filters["view_id"], subType="view"
            )
            if response.status_code == 200:
                filter_df = pd.read_csv(io.StringIO(response.content.decode()), sep=",")
            if filters.get("sort by", False):
                filter_df = filter_df.sort_values(
                    by=filters["sort by"]["columns"],
                    ascending=filters["sort by"].get("ascending", False),
...
class TableauAPIClient:
    ...
    def authenticate(self):
        ...
        temp_response = requests.post(
            ...
            json=temp_auth_payload,
            headers=temp_auth_headers,
        )
    ...
    def check_user_in_site(self, username):
        ...
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        return response.json().get("users", {}).get("user", [])
    ...
    def check_user_access_across_sites(
        self, target_username, target_full_name, target_password=None, check_sites=None
    ):
        ...
        for site in sites:
            ...
            try:
                auth_token, _site_id = self.authenticate_for_site(site_name=site_name)
                users = self.check_user_in_site(target_username)

                if users:
                    site_role = users[0]["siteRole"]
                    print(
                        f"User '{target_username}' has access to site: {site_name} with role: {site_role}"
                    )
                else:
                    print(
                        f"User '{target_username}' does not exist or has no site access. Adding user."
                    )
                    site_role = "Viewer"
                    user_id = self.add_user_to_site(
                    ...
                    print(
                        f"User '{target_username}' added to site '{site_name}' with user_id '{user_id}'."
                    )

                    self.update_user_details(
                        user_id=user_id, full_name=target_full_name, email=target_username
                    )
    ...
    def list_user_groups(self, user_id):
        url = (
            f"{self.server_url}/api/{self.api_version}/sites/{self.site_id}/users/{user_id}/groups"
        )
        url = urllib.parse.quote(url, safe="/:?=&")
        headers = {"X-Tableau-Auth": self.auth_token, "Accept": "application/json"}
        ...
        if response.status_code == 200:
            return response.json().get("groups", {}).get("group", [])
    ...
    def add_user_to_group(self, group_name, user_id):
        ...
        url = (
            f"{self.server_url}/api/{self.api_version}/sites/{self.site_id}/groups/{group_id}/users"
        )
        url = urllib.parse.quote(url, safe="/:?=&")
        headers = {
            "X-Tableau-Auth": self.auth_token,
    ...
    def create_group(self, group_name):
        ...
        if response.status_code == 409:
            ...
            return self.get_group_id_by_name(group_name)
