...
class DynamoDBHandler:
    ...
    def convert_dynamodb_values(self, attributes: List[Dict[str, Any]], attribute_values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attribute in attributes:
            ...
            if value is not None:
                if attribute_type == 'N' and isinstance(value, str):
                    ...
                    logger.warning(f"Unsupported attribute type '{attribute_type}' for attribute '{attribute_name}'")
                    continue

                converted_values[attribute_name] = dynamodb_to_python.get(attribute_type, str)(value)
    ...
    def get_table_metadata(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve table metadata: {e.response['Error']['Message']}")
            return {}

    def update_record(self, table_name: str, key: Dict[str, Any], attributes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to update record: {e.response['Error']['Message']}")
            raise

    def get_records(self, table_name: str, key_id: Optional[Any] = None, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if key_id is not None:
                ...
                while 'LastEvaluatedKey' in response:
                    response = table.query(KeyConditionExpression=Key('id').eq(key_id), ExclusiveStartKey=response['LastEvaluatedKey'])
                    items.extend(response.get('Items', []))

            elif filters:
                filter_expression = Attr(list(filters.keys())[0]).eq(list(filters.values())[0])
            ...
            logger.error(f"Error querying or scanning table {table_name}: {e.response['Error']['Message']}")
            return []

    def delete_all_records(self, table_name: str, limit: Optional[int] = None):
        """
        ...
        try:
            ...
            logger.error(f"Failed to delete items from {table_name}. Error: {e.response['Error']['Message']}")
            raise
    ...
    def push_record(self, table_name: str, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to push record to {table_name}. Error: {e.response['Error']['Message']}")
    ...
    def push_bulk_records(self, table_name: str, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to push bulk records to {table_name}. Error: {e.response['Error']['Message']}")
            return {'Error': e.response['Error']['Message']}
