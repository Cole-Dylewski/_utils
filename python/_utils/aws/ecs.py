...
class ECSHandler:
    ...
    def __init__(
        ...
        self.ecs_client = self.session.client(
            'ecs',
            region_name=self.region,
            endpoint_url=f'https://ecs.{self.region}.amazonaws.com'
        )
    ...
    async def monitor_task(
        ...
        while True:
            try:
                ...
                logger.error(f"Error monitoring ECS task {task_arn}: {e}")
                raise

    def stop_task(self, cluster_name: str, task_arn: str, reason: Optional[str] = None):
        """
        Stops an ECS task.
        """
        ...
        try:
            response = self.ecs_client.stop_task(
                cluster=cluster_name,
                task=task_arn,
                reason=reason or 'Stopped by ECSHandler'
            )
    ...
    def list_services(self, cluster_name: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Failed to list services in cluster {cluster_name}: {e}")
            raise

    def list_tasks(self, cluster_name: str, service_name: Optional[str] = None) -> List[str]:
        """
        ...
        try:
            ...
            logger.error(f"Failed to list tasks in cluster {cluster_name}: {e}")
            raise
    ...
    def get_cluster_full_config(self, cluster_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if cluster_response.get("clusters"):
                ...
                result["registeredContainerInstancesCount"] = cluster_meta.get("registeredContainerInstancesCount")
            ...
            for i in range(0, len(service_arns), 10):
                ...
                service_resp = self.ecs_client.describe_services(cluster=cluster_name, services=batch)
                ...
                for svc in service_resp.get("services", []):
                    ...
                    task_def_resp = self.ecs_client.describe_task_definition(taskDefinition=task_def_arn)
            ...
            logger.error(f"Failed to get full configuration for cluster {cluster_name}: {e}")
            raise
    ...
    def update_fargate_services(self, cluster_name: str, wait_interval: int = 30):
        ...
        try:
            ...
            logger.info(f"All services triggered for new deployment. Monitoring progress...")
            ...
            while not all_services_updated:
                ...
                for service in services:
                    ...
                    for deployment in service_desc.get('deployments', []):
                        ...
                        if rollout_state == 'FAILED':
                            logger.error(f"Deployment rollback detected for service {service} - Reason: {rollout_reason}")
                ...
                if not all_services_updated:
                    logger.info(f"Services are still updating. Waiting {wait_interval} seconds before checking again...")
            ...
            logger.error(f"AWS ClientError during service update: {str(e)}")
            raise

        except Exception as e:
            logger.error(f"Unexpected error during service update: {str(e)}")
            raise
