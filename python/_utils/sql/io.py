...
def dict_to_insert_stmt(log_args, schema, table):
    ...
    columnsList = [str(i) for i in log_args]
...
def df_to_insert_stmt(df, tableName, nullify=None, parseDataTypes=True, strip=False):
    ...
    for row in rows:
        ...
        for i, value in enumerate(row):
            ...
            # Remove unicode characters
                ...
                value = str(value)
                if parseDataTypes:
                    if (value.count("-") <= 1 and value.count(".") <= 1) and (
    ...
    values = "\n VALUES \n" + ",\n".join(values)
    return f"""INSERT INTO {tableName} {cols} {values};"""
...
def create_s3_copy_stmt(
    bucket, key, rds, schema, table, awsSecret, columnMap="", delimiter="", region="us-east-1"
):
    import os
...
def export_qry_to_s3(
    rds, query, bucket, objectKey, delimiter=",", region="us-east-1", returnS3Info=False
):
    if rds.lower() == "redshift":
        query, bucket, objectKey = unload_sql(
            query=query, objectKey=objectKey, bucket=bucket, delimiter=delimiter
        )

    if rds.lower() == "postgres":
        query, bucket, objectKey = export_sql(
            query=query, objectKey=objectKey, bucket=bucket, delimiter=delimiter, region=region
        )
