...
from cryptography.hazmat.primitives.ciphers import Cipher, algorithms as ciphers_algorithms, modes as ciphers_modes
...
from _utils.aws import secrets, s3
...
def encrypt_data(data, secret='rsaKeys'):
    ...
    try:
        ...
        # AES CBC Cipher
        aes_cbc_cipher = Cipher(ciphers_algorithms.AES(key), ciphers_modes.CBC(iv), backend=default_backend())
...
def decrypt_data(cypher_data, private_key='', secret='rsaKeys'):
    ...
    try:
        ...
        aes_cbc_cipher = Cipher(ciphers_algorithms.AES(recovered_key), ciphers_modes.CBC(cypher_data['iv']), backend=default_backend())
        decryptor = aes_cbc_cipher.decryptor()
        recovered_padded_plaintext = decryptor.update(cypher_data['ciphertext']) + decryptor.finalize()
...
def gen_rsa_keys(key_size=2048, secret_name='', region_name="us-east-1", key_format='pem', save_location=False, bucket='', client=''):
    """
        ...
        if save_location:
            ...
            if isinstance(save_location, str):
                if save_location.lower() == 's3':
                    s3_handler.send_to_s3(data = private_key, bucket = bucket, s3_file_name = f"keys/{client}/{client}_private_key.pem")
                    s3_handler.send_to_s3(data = public_key, bucket = bucket, s3_file_name = f"keys/{client}/{client}_public_key.pem")
