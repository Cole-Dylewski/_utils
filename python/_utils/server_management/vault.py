...
class VaultHandler:
    ...
    def __init__(
        ...
        kubernetes_roles: Optional[List[str]] = None
    ):
        """
        ...
        self.kubernetes_roles = kubernetes_roles or ["coder-workspace", "default", "coder", "workspace", "coder-workspace-role"]
    ...
    def _get_vault_token(self) -> Optional[str]:
        ...
        if sa_token_file.exists():
            try:
                ...
                with open(sa_token_file, "r") as f:
                    jwt = f.read().strip()
    ...
    def connect(self) -> Optional[hvac.Client]:
        """
        ...
        try:
            client = hvac.Client(url=self.vault_addr, token=token, verify=not self.vault_skip_verify)
            if not client.is_authenticated():
                logger.error("[VAULT] Vault authentication failed - token may be invalid or expired")
                return None
            logger.info("[VAULT] Successfully authenticated to Vault")
            self._client = client
            return client
        except Exception as e:
            logger.error(f"[VAULT] Error connecting to Vault at {self.vault_addr}: {e}")
    ...
    def get_secret(self, secret_name: str, path: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        ...
        client = self.connect()
        if not client:
            logger.debug(f"[VAULT] Cannot get secret: Vault client connection failed")
            return None
        ...
        try:
            ...
            if response and "data" in response and "data" in response["data"]:
                ...
                logger.debug(f"[VAULT] Successfully read secret from {full_path}, keys: {list(data.keys())}")
                return data
            else:
    ...
    def create_or_update_secret(
        ...
        client = self.connect()
        if not client:
            logger.error(f"[VAULT] Cannot create/update secret: Vault client connection failed")
            return False
        ...
        try:
            ...
            logger.error(f"[VAULT] Error creating/updating secret at {full_path}: {e}")
            logger.debug(f"[VAULT] Exception type: {type(e).__name__}")
            return False
    
    def delete_secret(self, secret_name: str, path: Optional[str] = None) -> bool:
        """
...
def main():
    ...
    parser.add_argument(
        "--base-path",
        default="cortex",
        help="Base path for secrets (default: cortex)"
    )
    parser.add_argument(
        "--vault-token",
        default=None,
    ...
    parser.add_argument(
        "--vault-host",
        default=None,
        help="Remote host for SSH token retrieval (e.g., user@host)"
    )
    parser.add_argument(
        "--token-path",
    ...
    put_parser.add_argument(
        "--kv", action="append", help="Key-value pair in format key=value (can be used multiple times)"
    )
...
def _cmd_put(args, parser):
    ...
    if args.kv:
        for kv_pair in args.kv:
            if "=" not in kv_pair:
                print(f"Error: Invalid key-value pair: {kv_pair}. Use format key=value", file=sys.stderr)
                sys.exit(1)
    ...
    if not success:
        print(f"[ERROR] Failed to create/update secret", file=sys.stderr)
        sys.exit(1)
...
def _cmd_delete(args, parser):
    ...
    if not args.force:
        response = input(f"Are you sure you want to delete secret/{handler.base_path}/{args.secret_name}? [y/N]: ")
        if response.lower() != "y":
            print("Cancelled.")
            return
    ...
    if not success:
        print(f"[ERROR] Failed to delete secret", file=sys.stderr)
        sys.exit(1)
