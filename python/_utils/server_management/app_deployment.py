...
class AppDeploymentConfig(ABC):
    ...
    def generate_missing_credentials(self, overwrite_existing: bool = False) -> Dict[str, str]:
        ...
        for cred_name in required:
            if cred_name == "database_password":
                ...
                if cred_name not in self.credentials.secrets or overwrite_existing:
    ...
    def get_vault_handler(self) -> Optional[VaultHandler]:
        """
    ...
    def load_credentials_from_vault(self) -> bool:
        ...
        try:
            ...
            if app_secrets:
                ...
                logger.debug(f"No application secrets found at: {vault.base_path}/{secrets_secret_name}")
        ...
        try:
            ...
            if self.credentials.database_password:
                ...
                if existing and not overwrite:
                    logger.info(f"[VAULT] Database secret already exists at {full_vault_path}, skipping (use overwrite=True to update)")
                    logger.debug(f"[VAULT] Existing secret keys: {list(existing.keys()) if existing else 'None'}")
                else:
                    ...
                    if success:
                        logger.info(f"[VAULT] Successfully saved database password to: {full_vault_path}")
                        # Verify the secret was saved
                        verification = vault.get_secret(database_secret_name)
                        if verification:
                            logger.info(f"[VAULT] Verification: Secret exists at {full_vault_path} with keys: {list(verification.keys())}")
                        else:
                            logger.warning(f"[VAULT] Verification failed: Secret not found at {full_vault_path} immediately after save")
                        saved_count += 1
                    else:
                        logger.error(f"[VAULT] Failed to save database password to: {full_vault_path}")
            ...
            if self.credentials.api_keys:
                ...
                if success:
                    ...
                    logger.error(f"Failed to save API keys to Vault")
...
class AppDeploymentManager(ABC):
    ...
    def __init__(
        ...
        self.terraform_handler: Optional[TerraformHandler] = None
        self.ansible_handler: Optional[AnsibleHandler] = None
    ...
    def provision_infrastructure(self) -> bool:
        ...
        try:
            ...
            if terraform.is_remote:
                logger.info(f"Ensuring remote terraform directory exists: {terraform.remote_project_dir}")
                import subprocess
                import os
                ...
                ssh_cmd.append(f'sudo mkdir -p {terraform.remote_project_dir} && sudo chown -R {self.config.server.user}:{self.config.server.user} {terraform.remote_project_dir}')
                ...
                if local_tf_files:
                    ...
                    cleanup_cmd.append(f"cd {terraform.remote_project_dir} && for f in *.tf; do if ! echo '{keep_files}' | grep -qw \"$f\"; then echo \"Removing extra file: $f\"; rm -f \"$f\"; fi; done")
                    try:
                ...
                terraform_source = str(self.terraform_dir).replace('\\', '/')
                # Copy all .tf files and README
                terraform_files = list(self.terraform_dir.glob("*.tf")) + list(self.terraform_dir.glob("README*"))
                ...
                for tf_file in terraform_files:
                    ...
                    try:
                        ...
                        logger.error(f"Terraform file copy timed out for {tf_file.name}")
                        return False
                    except FileNotFoundError:
                        logger.error("scp not found. Please install OpenSSH client.")
                        return False
                    except Exception as e:
                        logger.error(f"Failed to copy {tf_file.name}: {e}")
                        return False
            ...
            if terraform.is_remote:
                ...
                subprocess.run(ssh_cmd + [cleanup_cmd], capture_output=True, text=True, timeout=30)
            ...
            if init_result.returncode != 0:
                ...
                logger.debug(f"Terraform init stderr: {init_result.stderr[:1000] if init_result.stderr else 'None'}")
                logger.debug(f"Terraform init stdout: {init_result.stdout[:1000] if init_result.stdout else 'None'}")
                # Check for GPG/signature errors or dev_overrides issues in the output
                has_gpg_error = (
                    "key expired" in error_output.lower() or 
                    "signature" in error_output.lower() or 
                    "openpgp" in error_output.lower() or 
                    "expired" in error_output.lower() or
                    "error checking signature" in error_output.lower() or
                    "dev_overrides" in error_output.lower() or
                    "development overrides" in error_output.lower()
                )
                if has_gpg_error:
                    logger.warning("[TERRAFORM] Terraform init failed due to GPG signature issue, attempting workaround...")
                    if terraform.is_remote:
                        import subprocess
                        ssh_target = f"{self.config.server.user}@{self.config.server.host}"
                        logger.info("[TERRAFORM] Manually installing Docker provider to bypass GPG signature issue...")
                        logger.info("[TERRAFORM] This workaround downloads the provider directly from GitHub...")
                        ssh_cmd = ['ssh']
                        if self.config.server.ssh_key_path:
                            ssh_cmd.extend(['-i', self.config.server.ssh_key_path])
                        ssh_cmd.extend(['-p', str(self.config.server.port)])
                        ...
                        arch = "arm64" if "aarch64" in (arch_result.stdout or "").lower() or "arm64" in (arch_result.stdout or "").lower() else "amd64"
                        
                        # Get latest version from GitHub API (simpler and more reliable)
                        logger.info("[TERRAFORM] Fetching latest Docker provider version from GitHub...")
                        ...
                        provider_version = version_result.stdout.strip() if version_result.stdout and version_result.returncode == 0 else "3.9.0"
                        logger.info(f"[TERRAFORM] Using Docker provider version: {provider_version}")
...
TERRAFORMRC
  ...
  echo "Failed to download provider" && exit 1; \
                        ...
                        logger.info("[TERRAFORM] This may take 2-3 minutes to download and install...")
                        result = subprocess.run(ssh_cmd + [workaround_cmd], capture_output=True, text=True, timeout=600)
                        logger.info(f"[TERRAFORM] Provider installation exit code: {result.returncode}")
                        if result.stdout:
                            logger.info(f"[TERRAFORM] Provider installation output (last 20 lines):")
                            for line in result.stdout.split('\n')[-20:]:
                                if line.strip():
                                    logger.info(f"[TERRAFORM]   {line}")
                        if result.returncode == 0:
                            logger.info("[TERRAFORM] Terraform init succeeded with manually installed provider")
                            manually_installed_provider = True
                        else:
                            logger.warning(f"[TERRAFORM] Provider installation had issues: {result.stderr[:200] if result.stderr else 'No stderr'}")
                            error_msg = result.stderr or result.stdout or "Unknown error"
                            logger.error(f"[TERRAFORM] Manual provider installation failed: {error_msg[:500]}")
            ...
            if init_result.returncode == 0 or manually_installed_provider:
                ...
                if manually_installed_provider:
                    logger.info("[TERRAFORM] Note: Used manual provider installation workaround for GPG signature issue")
            else:
                logger.error(f"[TERRAFORM] Terraform initialization failed: {init_result.stderr[:500] if init_result.stderr else 'Unknown error'}")
                raise Exception(f"Terraform init failed: {init_result.stderr[:200] if init_result.stderr else 'Unknown error'}")
            ...
            if self.config.vault_config:
                ...
                logger.info(f"[TERRAFORM] Terraform will attempt to read Vault secrets at:")
                ...
                if self.config.vault_config:
                    ...
                    if vault:
                        logger.info(f"[TERRAFORM] Pre-flight check: Verifying Vault secrets exist...")
                        db_check = vault.get_secret(f"{environment}/database")
                        secrets_check = vault.get_secret(f"{environment}/secrets")
                        if db_check:
                            logger.info(f"[TERRAFORM] Pre-flight check: Database secret found at {expected_db_path}")
                        else:
                            logger.warning(f"[TERRAFORM] Pre-flight check: Database secret NOT found at {expected_db_path}")
                        if secrets_check:
                            logger.info(f"[TERRAFORM] Pre-flight check: Application secrets found at {expected_secrets_path}")
                        else:
                            logger.warning(f"[TERRAFORM] Pre-flight check: Application secrets NOT found at {expected_secrets_path}")
            
            # If we manually installed providers, use -lock=false to bypass lock file validation
            if manually_installed_provider:
                logger.info("Using -lock=false for plan/apply due to manually installed provider workaround")
                ...
                test_result = subprocess.run(ssh_cmd[:-1] + [docker_test_cmd], capture_output=True, text=True, timeout=10)
                ...
                logger.info("[TERRAFORM] Starting terraform plan (this may take several minutes for provider downloads)...")
                ...
                plan_result = subprocess.run(ssh_cmd, capture_output=True, text=True, timeout=900)
                
                plan_success = plan_result.returncode == 0 or plan_result.returncode == 2
                ...
                if plan_result.returncode == 0:
                    ...
                    logger.error(f"[TERRAFORM] Terraform plan failed with return code {plan_result.returncode}")
                    raise Exception(f"Terraform plan failed: {plan_result.stderr[:200] if plan_result.stderr else 'Unknown error'}")
                
                # Run apply with -lock=false
                # Build terraform command with all variables
                apply_cmd_parts = [f'cd {terraform.remote_project_dir} && terraform apply -auto-approve -lock=false']
                for key, value in terraform_vars.items():
                    if isinstance(value, (dict, list)):
                        import json
                        value = json.dumps(value)
                ...
                ssh_cmd_apply = ssh_cmd[:-1] + [apply_cmd_with_sudo]  # Reuse ssh_cmd but replace the command
                logger.info("[TERRAFORM] Running terraform apply with sudo docker access...")
                logger.info("[TERRAFORM] Progress will be logged as it becomes available...")
                apply_result = subprocess.run(ssh_cmd_apply, capture_output=True, text=True, timeout=900)
                ...
                if apply_result.returncode == 0:
                    ...
                    logger.error(f"[TERRAFORM] Terraform apply failed with return code {apply_result.returncode}")
            else:
                ...
                if terraform.is_remote:
                    ...
                    plan_cmd_with_sudo = f'sudo sh -c "cd {terraform.remote_project_dir} && {plan_cmd}"'
                    ssh_cmd.append(plan_cmd_with_sudo)
                    logger.info("[TERRAFORM] Starting terraform plan (this may take several minutes)...")
                    plan_result = subprocess.run(ssh_cmd, capture_output=True, text=True, timeout=900)
                    plan_success = plan_result.returncode == 0 or plan_result.returncode == 2
                    ...
                    if plan_result.stdout:
                        if "no secret found" in plan_result.stdout.lower() or ("error" in plan_result.stdout.lower() and "vault" in plan_result.stdout.lower()):
                            logger.error("[TERRAFORM] Vault-related error detected in plan output:")
                            for line in plan_result.stdout.split('\n'):
                                if ("error" in line.lower() and "vault" in line.lower()) or "no secret found" in line.lower():
                    ...
                    # Wrap in sh -c to handle cd and other shell built-ins
                    logger.info("[TERRAFORM] Building terraform apply command...")
                    apply_cmd_with_sudo = f'sudo sh -c "cd {terraform.remote_project_dir} && {apply_cmd}"'
                    ...
                    if plan_result.returncode != 0:
                        ...
                        if plan_result.stdout:
                            ...
                            if "no secret found" in plan_result.stdout.lower() or "vault" in plan_result.stdout.lower():
                                logger.error("[TERRAFORM] Vault-related error detected in plan output:")
                                for line in plan_result.stdout.split('\n'):
                                    if "vault" in line.lower() or "secret" in line.lower() or "error" in line.lower():
                                        logger.error(f"[TERRAFORM]   {line}")
    ...
    def install_basic_dependencies(self) -> bool:
        ...
        try:
                ...
                if check_result.returncode == 0 and "NOT_INSTALLED" not in check_result.stdout:
                    ...
                    if version_match:
                        ...
                        if version_parts < [1, 14, 1]:
                            logger.info(f"Terraform v{current_version} is outdated, will upgrade to 1.14.1")
                        ...
                        logger.info("Terraform is installed but version could not be determined, will upgrade")
                        terraform_needs_upgrade = True
            except Exception as e:
            ...
            try:
                ...
                ansible_installed = ansible_check_result.returncode == 0 and 'ansible' in ansible_check_result.stdout
                if not ansible_installed:
                    ...
                    if ansible_install_result.returncode == 0:
                        ...
                        if ansible_install_result.stdout:
                            logger.debug(f"Ansible version: {ansible_install_result.stdout.strip()}")
                    else:
                        logger.warning(f"Ansible installation may have failed: {ansible_install_result.stderr[:200]}")
                else:
...
terraform version"""
            ...
            try:
                ...
                logger.error("Terraform installation timed out")
                return False
            except Exception as e:
            ...
            logger.error(f"Basic dependency installation failed: {e}")
            return False
    ...
    def install_dependencies(self) -> bool:
        ...
        try:
            ...
            if ansible.is_remote:
                ...
                subprocess.run(ssh_cmd + [ssh_target, mkdir_cmd], capture_output=True, text=True, timeout=30)
                ...
                copy_result = subprocess.run(scp_cmd, capture_output=True, text=True, timeout=60)
...
"""
                ...
                write_inv_cmd = f"cat > {inventory_path} << 'INVENTORY_EOF'\n{localhost_inventory}INVENTORY_EOF"
                subprocess.run(ssh_cmd + [ssh_target, write_inv_cmd], capture_output=True, text=True, timeout=30)
...
"""
    ...
    def deploy(
        ...
        logger.info(
            f"{self.config.app_name} deployment pipeline completed successfully"
        )
        return True
    ...
    def destroy(self) -> bool:
        ...
        """
        logger.warning(
            f"Destroying {self.config.app_name} deployment "
            f"({self.config.environment.value})..."
        )
